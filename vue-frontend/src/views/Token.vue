...
import * as Artifact from "../../../L2-zksync/artifacts-zk/contracts/zkToken.sol/zkToken.json";
...
const transfer = async () => {
  //@ts-ignore
  const erc20Contract = new Contract(ERC20_ADDRESS, Artifact.abi, new Web3Provider(window.ethereum).getSigner());

  try {
    btnUpdate.value = "...";
    const amountFormatted = ethers.utils.parseUnits(amount.value, 18);
    const tx = await erc20Contract.transfer(destinationAccount.value, amountFormatted);
...
const mint = async () => {
  ...
  try {
    ...
    const tx = await erc20Contract.mint(destinationAccount.value, amountFormatted);
